    ...
    const searchFilterFunction = useCallback((text: string) => {
        ...
        if (text) {
            ...
            const newData = assetEntries.filter((item) => {
                ...
                const combinedFields = `${item.natureofailment} ${item.medicineprescribed} ${item.procedureundertaken} ${item.clinicMonth + 1} ${moment([item.clinicYear, item.clinicMonth, item.clinicDay]).format("LL")}`;
