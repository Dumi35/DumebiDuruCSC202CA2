...
const Spreadsheet: React.FC<Props> = ({ entries }) => {
    ...
    const makeTable = () => {
        ...
        const table = {
            ...
            headers: entriesToShare.length > 0? ["", "S/N", "Clinic Date", "Nature of Ailment", "Medicine Precribed", "Procedure Undertaken"]: [],//first empty space is for id in row which is not shown but kept for reference
            rows: entriesToShare.map((entry) => {
                return [entry.id, entry.SN, moment([entry.clinicYear!, entry.clincMonth!, entry.clinicDay!]).format("LL"), entry.natureofailment, entry.medicineprescribed , entry.procedureundertaken, entry.nextappdate ]
            }),
    ...
    return (
        <View style={styles.container}>
            <View style={[styles.inputContainerStyle, { flexDirection: 'row', justifyContent: 'space-between', backgroundColor: "lightpink" }]}>
...
const styles = StyleSheet.create({
    ...
    row: { flexDirection: 'row', backgroundColor: 'white', borderWidth: 1, borderColor: 'white' },
    text: { padding: 6 },
    ...
    cellBorders: {
        borderWidth: 1, borderColor: 'pink', height: '100%'
    }
});