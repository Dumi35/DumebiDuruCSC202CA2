...
const Spreadsheet: React.FC<Props> = ({ entries }) => {
    ...
    const makeTable = () => {
        ...
        const table = {
            ...
            headers: entriesToShare.length > 0? ["Date of birth", "First Name","Middle Name","Surname", "Address","Date of Registration", "20120612141"] : [],//first empty space is for id in row which is not shown but kept for reference
            rows: entriesToShare.map((entry) => {
                return [moment([entry.birthYear!, entry.birthMonth!, entry.birthDay!]).format("LL"), entry.firstname, entry.middlename, entry.surname, entry.address, entry.regdate, entry.matricno ? 'Yes' : 'No']
            }),
...
const styles = StyleSheet.create({
    ...
    row: { flexDirection: 'row', backgroundColor: 'white', borderWidth: 1, borderColor: 'pink' },
    text: { padding: 6 },
