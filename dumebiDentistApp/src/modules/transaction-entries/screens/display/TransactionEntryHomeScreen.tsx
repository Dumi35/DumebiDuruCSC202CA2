    ...
    const searchFilterFunction = useCallback((text: string) => {
        ...
        if (text) {
            ...
            const newData = transactionEntries.filter((item) => {
                ...
                const combinedFields = `${item.firstname} ${item.middlename} ${item.surname}${item.address} ${item.regdate} ${item.matricno} ${item.birthMonth + 1} ${moment([item.birthYear, item.birthMonth, item.birthDay]).format("LL")}`;
