...
/**
 ...
 export const transformEntriesToDateSections = (entries: ITransactionEntry[]): EntriesInDateSections[] => {
    ...
    const distinctTxnDates = [...new Set(entries.map(entry => {
      const birthDate = moment([entry.birthYear!, entry.birthMonth!, entry.birthDay!]).format('LL');
      return birthDate;
    }))];
    ...
    const entryByDates: EntriesInDateSections[] = distinctTxnDates.map((distinctBirthDate) => {

      let dataOnBirthDate: ITransactionEntry[] = [];
      entries.map((entry) => {
        const birthDate = moment([entry.birthYear!, entry.birthMonth!, entry.birthDay!]).format('LL');
        if (birthDate == distinctBirthDate) {
          dataOnBirthDate.push(entry)
        }
      })
      return { title: distinctBirthDate, data: dataOnBirthDate }
